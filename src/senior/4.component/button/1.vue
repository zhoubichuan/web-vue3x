    ...
    <vue-tree :data="schoolBooks">
            ...
            <div class="root-content light-blue">
                <div class="expand-wrapper" @click="handleLoad(expand, handleExpand, row)">
                    ...
                    <div class="detail-content">
                        <span class="title">{{ row.bookName }}</span>
                    </div>
                </div>
            </div>
        </template>
        <template #branch="{ row, expand, handleExpand }">
            <div class="branch-content node-content" @click="handleLoad(expand, handleExpand, row)">
                ...
                <div class="detail-content">
                    <span class="title">{{ row.chapterName }}(共{{ row.qstCount }}题)</span>
                </div>
            </div>
        </template>
        <template #leaf="{ row }">
            <div class="leaf-content node-content">
                <div class="text">
                    <div class="title">{{ row.chapterName }}</div>
                    <div class="sub-title">{{ row.chapterName }}</div>
                </div>
                <div class="handle" v-if="!row.children || !row.children.length">
                    <button class="button" @click="scanAndCorrect(row)">xxxx</button>
                    <button class="button" :class="{ disabled: !row.studyTaskId }" @click="viewReport(row)">xx</button>
                </div>
            </div>
        </template>
    </vue-tree>
...
const { isLoading, request, schoolBooks, myPapers } = {
    isLoading: true,
    request: () => {
        myPapers = [{ label: "1111" }];
    },
...
const loadNode = async (nodeProps: any) => {
    ...
    const childes = apiRes.data.map((item) => ({
        id: item.nodeId,
        // @ts-ignore
        label: item.chapterName || item.pageName,
        children: [],
        ...item,
    }));
