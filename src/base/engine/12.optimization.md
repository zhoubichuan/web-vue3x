...
- 简化路径不仅可以提高性能、而且能够非常友好的对于开发者和协同人员进行阅读，而且还能避免使用`../`路径出错的风险
  ...
  ```
  ::: warning 在 less, sass, stylus 中，使用 @import "@/style/theme" 的语法引用目录确会报错。解决办法是是在引用路径的字符串最前面添加上 ~ 
...
## 6.图片压缩
当图片比较大时（如：大于1M）,可以先在压缩网站将图片体积压缩，然后使用 [https://imagecompressor.com/zh/](https://imagecompressor.com/zh/)
